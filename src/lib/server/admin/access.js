...
// challenges the visitor at the edge (GitHub/Google via the Zero Trust IdP) and
...
/** The Access token Cloudflare attached to this request event, if any. */
export const accessTokenFrom = (event) =>
  event.request.headers.get('cf-access-jwt-assertion') || event.cookies.get('CF_Authorization') || null;

/** Gate a request via Access. { ok: true, email } or { ok: false, status: 403, error }. */
export async function requireAccess(event) {
  try {
    const payload = await verifyAccessToken(accessTokenFrom(event));
